<script lang="ts">
	...
	function getPriorityColor(priority?: number): { bg: string; border: string; text: string } {
		switch (priority) {
			case 3:
				return {
...
<div
	class={cn(
		"absolute left-0 right-0 rounded-lg border px-2 py-1 cursor-pointer transition-all",
