<script lang="ts">
	...
	const tooltipContent = $derived(
		tasks
			.map((t) => {
				const priority = t.priority ? ` [${t.priority === 3 ? "High" : t.priority === 2 ? "Med" : "Low"}]` : "";
...
<Tooltip.Provider>
	<Tooltip.Root>
		...
		>
			<!-- Outlined circle at the left -->
			<div
				class={cn(
					"absolute left-0 size-3 rounded-full border-2",
					getPriorityColor(tasks[0]?.priority),
					"border-background bg-background -ml-1.5 z-10",
				)}
			></div>
			...
			{#if tasks[0]?.completed}
				<div class="absolute left-0 -ml-1.5 z-20">
					<CheckIcon class="size-3 text-muted-foreground bg-background rounded-full" />
				</div>
			{/if}
		</Tooltip.Trigger>
