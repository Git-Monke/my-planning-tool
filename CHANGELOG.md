...
- **Keyboard navigation:** single-letter shortcuts **E** → Tasks (`/tasks`), **R** → Calendar (`/calendar`), **T** → Notes (`/notes`) when not typing in a field and without Ctrl/Meta/Alt held (browser shortcuts unchanged).
...
- **Sidebar main nav:** removed **collapsed-state tooltips** on Tasks / Calendar / Notes (labels appear on hover-expand; tooltips were redundant).
